...
def has_repeated_content(s: str, threshold: float = 0.3, tail_length: int = 1000) -> bool:
    ...
    log_file_path = os.path.join(log_dir, 'rl_repeated_content_orm.txt')
...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _move_model_to_vllm(self):
        # NOTE(safety-thyme): unconditional entry probe to confirm this method
        # is actually called (previous debug attempts were gated by conditions
        # that silently short-circuited).
        if getattr(self, 'accelerator', None) is not None and \
                self.accelerator.process_index == 0 and \
                not getattr(self, '_mmv_entry_logged', False):
            print('[safety-thyme] _move_model_to_vllm ENTER '
                  f'vllm_mode={getattr(self, "vllm_mode", "?")} '
                  f'use_fast_infer={getattr(self, "use_fast_infer", "?")} '
                  f'use_vllm={getattr(self.args, "use_vllm", "?")}',
                  flush=True)
            self._mmv_entry_logged = True
        if self.vllm_mode == 'server':
        ...
        for i, parameter_group in enumerate(self.parameter_groups):
            ...
            with unwrap_model_for_generation(
                ...
                if self.use_fast_infer:
                    ...
                    # NOTE(safety-thyme): unconditional diagnostic — always print
                    # a sample of state_dict keys on rank 0 the first time we
                    # enter this branch. This proves the code path is executed
                    # and shows the exact key layout that vllm.load_weights sees.
                    if getattr(self, 'accelerator', None) is not None and \
                            self.accelerator.process_index == 0 and \
                            not getattr(self, '_sd_sample_logged', False):
                        _all_keys = list(state_dict.keys())
                        _prefixes = sorted({(k.split('.', 1)[0] if isinstance(k, str) else type(k).__name__)
                                            for k in _all_keys})
                        _visual_sample = [k for k in _all_keys if isinstance(k, str) and 'visual' in k][:3]
                        print(f'[safety-thyme] state_dict before load: '
                              f'num_keys={len(_all_keys)} '
                              f'top_prefixes={_prefixes} '
                              f'first5={_all_keys[:5]} '
                              f'visual_sample={_visual_sample} '
                              f'llm_model_cls={type(llm_model).__name__} '
                              f'has_visual={hasattr(llm_model, "visual")} '
                              f'has_language_model={hasattr(llm_model, "language_model")}',
                              flush=True)
                        self._sd_sample_logged = True
                    # NEW STRATEGY (safety-thyme): bypass vllm's hf_to_vllm_mapper
                    # entirely. It has ordering-dependent prefix matching that
                    # mis-routes ``model.visual.*`` params into the Qwen2 LLM
                    # loader (see qwen2.py:412 KeyError). Instead, partition
                    # state_dict by *module ownership* and hand each partition
                    # directly to the corresponding submodule's load_weights.
                    _did_split_load = False
                    _is_qwen25_vl = (
                        type(llm_model).__name__ == 'Qwen2_5_VLForConditionalGeneration'
                        and hasattr(llm_model, 'visual')
                        and hasattr(llm_model, 'language_model')
                    )
                    if _is_qwen25_vl:
                        try:
                            _visual_w = []      # (name_relative_to_visual, tensor)
                            _lm_w = []          # (name_relative_to_language_model, tensor)
                            _leftover_w = []    # anything else
                            for _k, _v in state_dict.items():
                                if not isinstance(_k, str):
                                    _leftover_w.append((_k, _v))
                                    continue
                                # New (transformers>=5): ``model.visual.*`` / ``model.language_model.*``
                                # Old (transformers<5): ``visual.*``           / ``model.*``
                                if _k.startswith('model.visual.'):
                                    _visual_w.append((_k[len('model.visual.'):], _v))
                                elif _k.startswith('visual.'):
                                    _visual_w.append((_k[len('visual.'):], _v))
                                elif _k.startswith('model.language_model.'):
                                    # inside language_model, keep the ``model.`` prefix
                                    _lm_w.append(('model.' + _k[len('model.language_model.'):], _v))
                                elif _k == 'lm_head.weight' or _k.startswith('lm_head.'):
                                    # lm_head belongs to language_model in vllm
                                    _lm_w.append((_k, _v))
                                elif _k.startswith('model.'):
                                    # flat ``model.embed_tokens`` / ``model.layers.*`` — belongs to LLM
                                    _lm_w.append((_k, _v))
                                else:
                                    _leftover_w.append((_k, _v))
                            if getattr(self, 'accelerator', None) is not None and \
                                    self.accelerator.process_index == 0 and \
                                    not getattr(self, '_sd_split_logged', False):
                                print(f'[safety-thyme] split state_dict: '
                                      f'visual={len(_visual_w)} '
                                      f'language_model={len(_lm_w)} '
                                      f'leftover={len(_leftover_w)}',
                                      flush=True)
                                if _visual_w:
                                    print(f'[safety-thyme]   visual first3: '
                                          f'{[n for n,_ in _visual_w[:3]]}', flush=True)
                                if _lm_w:
                                    print(f'[safety-thyme]   lm first3: '
                                          f'{[n for n,_ in _lm_w[:3]]}', flush=True)
                                self._sd_split_logged = True
                            if _visual_w and hasattr(llm_model.visual, 'load_weights'):
                                llm_model.visual.load_weights(iter(_visual_w))
                            elif _visual_w:
                                # visual has no load_weights; fall through to
                                # generic AutoWeightsLoader (below).
                                _lm_w.extend([('visual.' + n, v) for n, v in _visual_w])
                            if _lm_w:
                                llm_model.language_model.load_weights(iter(_lm_w))
                            _did_split_load = True
                            if getattr(self, 'accelerator', None) is not None and \
                                    self.accelerator.process_index == 0 and \
                                    not getattr(self, '_sd_split_ok_logged', False):
                                print('[safety-thyme] split load_weights OK',
                                      flush=True)
                                self._sd_split_ok_logged = True
                        except BaseException as _split_exc:
                            if getattr(self, 'accelerator', None) is not None and \
                                    self.accelerator.process_index == 0 and \
                                    not getattr(self, '_split_err_logged', False):
                                import traceback as _tb_split
                                print('===== [safety-thyme] split load FAILED, falling back =====',
                                      flush=True)
                                print(f'type={type(_split_exc).__name__}  msg={_split_exc!r}',
                                      flush=True)
                                _tb_split.print_exc()
                                print('===========================================================',
                                      flush=True)
                                self._split_err_logged = True
                            _did_split_load = False
                    # Fallback path: use vllm's own load_weights (original code).
                    if not _did_split_load:
                        try:
                            llm_model.load_weights(state_dict.items())
                        except BaseException as _lw_exc:
                            if getattr(self, 'accelerator', None) is not None and \
                                    self.accelerator.process_index == 0 and \
                                    not getattr(self, '_lw_exc_logged', False):
                                import traceback as _tb2
                                print('===== [safety-thyme] load_weights RAISED =====',
                                      flush=True)
                                print(f'type={type(_lw_exc).__name__}  msg={_lw_exc!r}',
                                      flush=True)
                                _tb2.print_exc()
                                print('==============================================',
                                      flush=True)
                                self._lw_exc_logged = True
                            raise
                    del state_dict
                    gc_collect()
    ...
    def process_round(self, gen_inputs, gen_outputs, previous_execution_contexts, cur_round=1000):
        ...
        cache_dir = os.path.join(self.args.output_dir, 'RL_case_cache', f"{self.start_time}_{self.accelerator.process_index}")
    ...
    def _o3_infer(self, inputs: InputsType) -> Tuple[InputsType, OutputsType]:
        ...
        while iteration_limit > 0:
            ...
            try:
                ...
                if self.accelerator.process_index == 1:
                    print(f"OS Error: {e}")
                _dbg_dir = os.path.join(self.args.output_dir, 'debug')
                os.makedirs(_dbg_dir, exist_ok=True)
                with open(os.path.join(_dbg_dir, 'debug_os_error.jsonl'), "a") as fout:
                ...
                _dbg_dir = os.path.join(self.args.output_dir, 'debug')
                os.makedirs(_dbg_dir, exist_ok=True)
                with open(os.path.join(_dbg_dir, 'debug_oom.jsonl'), "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                _dbg_dir = os.path.join(self.args.output_dir, 'debug')
                os.makedirs(_dbg_dir, exist_ok=True)
                with open(os.path.join(_dbg_dir, 'debug_value.jsonl'), "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                _dbg_dir = os.path.join(self.args.output_dir, 'debug')
                os.makedirs(_dbg_dir, exist_ok=True)
                with open(os.path.join(_dbg_dir, 'debug_idx.jsonl'), "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                _dbg_dir = os.path.join(self.args.output_dir, 'debug')
                os.makedirs(_dbg_dir, exist_ok=True)
                with open(os.path.join(_dbg_dir, 'debug_oot.jsonl'), "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                _dbg_dir = os.path.join(self.args.output_dir, 'debug')
                os.makedirs(_dbg_dir, exist_ok=True)
                with open(os.path.join(_dbg_dir, 'debug_runtime.jsonl'), "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
                ...
                if self.accelerator.process_index == 1:
                    print(f"Exception: {e}")
                # NOTE(safety-thyme): also dump the full traceback (rank-0 only,
                # capped to first N occurrences) so we can see where the
                # exception is actually raised from — printing just str(e)
                # loses the traceback and made earlier debugging pointless.
                if self.accelerator.process_index == 0 and \
                        getattr(self, '_o3_exc_tb_dumps', 0) < 3:
                    import traceback as _tb
                    print('===== [safety-thyme] _o3_infer caught Exception =====',
                          flush=True)
                    print(f'type={type(e).__name__}  msg={e!r}', flush=True)
                    _tb.print_exc()
                    print('=====================================================',
                          flush=True)
                    self._o3_exc_tb_dumps = getattr(self, '_o3_exc_tb_dumps', 0) + 1
                _dbg_dir = os.path.join(self.args.output_dir, 'debug')
                os.makedirs(_dbg_dir, exist_ok=True)
                with open(os.path.join(_dbg_dir, 'debug_others.jsonl'), "a") as fout:
                    fout.write(f"Wrong Case: {e}\n")
                    for i in range(len(gen_inputs)):
                        fout.write(json.dumps(gen_inputs[i]) +"\n")
                        fout.write(json.dumps(gen_outputs[i]) +"\n")
    ...
    def _score_completions(self, inputs: InputsType) -> Tuple[torch.Tensor, torch.Tensor, List[str]]:
        ...
        # -------- TFA (Teacher-Forced Visual Ablation) side-computation --------
        # If TFA_ENABLE=1 AND ``safety_tfa_tool_orm`` is among reward_funcs,
        # run two teacher-forced forwards per sample (orig image vs. same-
        # size white image at the t-th sandbox tool call) and stash the raw
        # D_t values on each example so that TFAToolORM can bucketize +
        # weight them below. Failure modes (missing crop, OOM, shape
        # mismatch, ...) fall back to per-sample D_t = None; the ORM then
        # returns 0 for those samples and training continues.
        _r = self.accelerator.process_index
        try:
            self._compute_tfa_rewards(inputs)
        except Exception as _tfa_e:  # defensive: never let TFA kill a step
            print(f"[tfa][error] _compute_tfa_rewards crashed; skipping "
                  f"this step: {type(_tfa_e).__name__}: {_tfa_e}", flush=True)
            import traceback as _tb
            _tb.print_exc()
            for _ex in inputs:
                _ex.setdefault("tfa_D_per_sample", None)
        ...
        for i, (reward_func, reward_model_plugin) in enumerate(zip(self.reward_funcs, self.reward_model_plugins)):
            ...
            # NOTE(safety-thyme): Thyme's original logic assumed the plugin file
            # was named ``agent_rm.py`` and parsed ``<agent_rm.VQAORM object at
            # 0x...>``. When the plugin lives in a different file (ours is
            # ``safety_rm.py``) that string prefix does not match and the
            # downstream ``reward_name_list.index('VQAORM')`` fails. Prefer the
            # class name directly (falls back to the legacy parse for
            # non-class rewards, e.g. plain functions).
            _cls = getattr(reward_func, '__class__', None)
            if isinstance(reward_func, nn.Module) is False and _cls is not None and _cls is not type:
                _n = _cls.__name__
            else:
                _n = reward_name.split(" object at")[0].replace("<agent_rm.", "").strip()
            reward_name_list.append(_n)
        ...
        if (total_rewards_per_func_before - total_rewards_per_func).sum() != 0:
            # WARNING: writing to a single shared path from every rank on a
            # distributed FS (CephFS) caused a silent, deterministic hang
            # here (all ranks passed `gather` but none reached the TFA gate).
            # Only rank 0 writes, and we tag the filename with the rank to be
            # extra safe, and never let a save failure block training.
            try:
                _dbg_dir = os.path.join(self.args.output_dir, 'debug')
                os.makedirs(_dbg_dir, exist_ok=True)
                _save_path = os.path.join(_dbg_dir, 'add_mask.pth')
                if _r == 0:
                    torch.save(data_to_save, _save_path)
                # NOTE: we intentionally do NOT broadcast/barrier here; the
                # save is purely diagnostic and must not become a collective.
            except Exception as _save_e:
                print(f"[tfa][warn] rank {_r}: add_mask save FAILED "
                      f"({type(_save_e).__name__}: {_save_e}); skipping",
                      flush=True)

        # -------- TFA quality gate: DISABLED (20260805) --------
        # Previously we filtered +bucket samples with (correct AND F>=F_high).
        # The gate combined with a small D_t distribution and low VQA/F rates
        # squashed effective TFAToolORM/mean to ~0.01 (0.7% of total reward,
        # below GRPO group-advantage noise floor). Per user decision, ALL
        # gating is now removed: whatever TFAToolORM.__call__ returned
        # (which is already positive-only after the 20260804 bucket change)
        # flows through to the reward sum unchanged. A single diagnostic
        # line still reports the +bucket count so we can monitor activation.
        if "TFAToolORM" in reward_name_list:
            tfa_idx = reward_name_list.index("TFAToolORM")
            tfa_col = total_rewards_per_func[:, tfa_idx]
            _n_pos = int((tfa_col > 0).sum().item())
            _n_zero_gate = int((tfa_col == 0).sum().item())
            _tfa_mean = float(tfa_col.mean().item())
            print(
                f"[tfa][gate] DISABLED (no correctness/F filter): "
                f"pos={_n_pos}  zero={_n_zero_gate}  "
                f"tfa_col_mean={_tfa_mean:+.4f}",
                flush=True,
            )
    ...
    # ------------------------------------------------------------------
    # TFA (Teacher-Forced Visual Ablation) — per-sample D_t computation.
    # ------------------------------------------------------------------
    def _compute_tfa_rewards(self, inputs: InputsType) -> None:
        """Populate ``inputs[i]['tfa_D_per_sample']`` in-place.

        For each sample that has at least one sandbox tool call, we do
        two teacher-forced forwards under ``self.model.eval()`` +
        ``no_grad``:

            (a) orig  -- with the original crops
            (b) cf    -- with same-size white images replacing the
                         t-th crop (t = 0 .. TFA_MAX_TOOLS_PER_SAMPLE-1)

        We compute ``D_t = mean(orig_logps - cf_logps)`` inside the post-
        tool token window ``M_t`` (see 文档 §3.2). All values that end up
        stashed on ``inputs[i]['tfa_D_per_sample']`` are raw ``D_t`` in
        nats/token; bucketization and quality-gating happen in
        ``TFAToolORM.__call__`` and ``_score_completions`` respectively.

        Fail-open: on ANY exception for a given sample we set that
        sample's value to ``None`` (=> TFAToolORM returns 0 for it).
        """
        # Fast path: TFA disabled or ORM not registered.
        if int(os.environ.get("TFA_ENABLE", "0")) != 1:
            for ex in inputs:
                ex.setdefault("tfa_D_per_sample", None)
            return
        tfa_names = {"safety_tfa_tool_orm"}
        reward_func_names = {getattr(f, "__class__", type(f)).__name__ for f in self.reward_funcs}
        # Also accept the swift-registered short name via reward_funcs attr.
        # If nobody registered TFAToolORM we still let the loop run so the
        # offline probe stays runnable, but the ORM returns 0 anyway.

        # ---- Bring shared TFA primitives into scope (once per step) ----
        try:
            _sft_dir = os.environ.get(
                "TFA_SFT_DIR",
                os.path.join(
                    os.path.dirname(os.path.abspath(__file__)),
                    "..", "..", "..", "data_synth", "safety_sft",
                ),
            )
            import sys as _sys
            if _sft_dir not in _sys.path:
                _sys.path.insert(0, _sft_dir)
            ...
            print(f"[tfa][error] cannot import cf_faith.tfa_core: "
                  f"{type(e).__name__}: {e}. Setting all D_t=None.",
                  flush=True)
            for ex in inputs:
                ex.setdefault("tfa_D_per_sample", None)
            return

        max_tools = int(os.environ.get("TFA_MAX_TOOLS_PER_SAMPLE", "1"))
        K_default = int(os.environ.get("TFA_WINDOW_K", "128"))
        white_root = os.environ.get(
            "TFA_WHITE_ROOT",
            os.path.join(_sft_dir, "_tfa_white_cache"),
        )
        os.makedirs(white_root, exist_ok=True)

        template = self.template
        tokenizer = template.tokenizer if hasattr(template, "tokenizer") else self.processing_class
        device = self.accelerator.device

        # NOTE: do NOT call ``self.model.eval()`` / ``self.model.train()``
        # here. Under DeepSpeed ZeRO-3 + ``offload_model=true`` these calls
        # trigger the ZeRO parameter-state machine (all-gather / partition
        # and their registered hooks) on every rank. Because this method
        # runs right before the ``gather(rewards_per_func)`` all_gather in
        # ``_score_completions``, the extra ZeRO NCCL traffic can interleave
        # with / desync from that collective and DEADLOCK the whole step
        # (observed: deterministic hang right after the ORM loop, before
        # [tfa][gate]). The TFA forwards are already wrapped in
        # ``no_grad`` (see below), so leaving the model in its current
        # mode is both safe and correct for log-prob computation.
        # _was_training = self.model.training   # no longer toggled

        # We need template in an encode-friendly mode. Training-time
        # template is set up by ms-swift already; we do NOT call
        # ``set_mode`` here because that would mutate global template
        # state (grpo_trainer switches template mode internally). The
        # important thing is that we round-trip via
        # ``template.encode + template.data_collator([...]) `` which is
        # the same code path ``_prepare_batch_inputs`` uses.
        # We deliberately do NOT call ``template.set_mode(...)`` here.
        # ms-swift's Template is a process-global singleton; set_mode()
        # reinitializes its internal mode/state and, under vLLM colocate +
        # multi-rank, each rank doing set_mode("train") -> set_mode(backup)
        # can leave ranks with divergent template state and desync the
        # subsequent ``gather(rewards_per_func)`` all_gather (=> deterministic
        # hang right after the ORM loop, before [tfa][gate]). TFA only needs
        # input_ids/pixel_values from encode+data_collator, which are
        # identical across train/eval modes, so leaving the mode untouched
        # is correct.
        template_mode_backup = getattr(template, "mode", None)
        template_model_backup = getattr(template, "model", None)
        template.model = self.model    # required by qwen._get_position_ids

        _n_pos = _n_neg = _n_zero = _n_skip = _n_fail = 0
        _t_start = time.time()
        _r = self.accelerator.process_index

        # =====================================================================
        # BATCH-SYMMETRIC TFA FORWARDS  (ZeRO-3-safe by construction)
        # =====================================================================
        # Rationale
        # ---------
        # The old per-sample loop deadlocked under DeepSpeed ZeRO-3 + offload
        # because different ranks called ``model(...)`` a different number of
        # times (ranks whose batch had tool crops did TFA forwards; ranks
        # whose batch had none did zero). Each ZeRO-3 forward triggers a
        # parameter all-gather collective that ALL ranks must join in lock-
        # step, so uneven forward counts across ranks -> hang.
        #
        # New design: every rank makes EXACTLY 2 forwards per step
        #   (a) orig  = policy log-prob under the true rollout (all images)
        #   (b) cf    = policy log-prob under a *counterfactual* rollout in
        #               which each sample's FIRST sandbox crop is replaced by
        #               a same-size WHITE image (samples with no crop are
        #               passed through unchanged, so their cf == orig and
        #               D_t == 0, which correctly maps to bucket 0 = no
        #               tool reward).
        # Both forwards go through ``self._get_per_token_logps`` which is
        # exactly the code path ms-swift already uses for ``old_per_token_logps``
        # and ``ref_per_token_logps`` -- so we inherit its ZeRO-3 correctness
        # for free.
        # =====================================================================
        try:
            ...
            # -------- Build orig-request list and, in parallel, the cf list --------
            orig_reqs: List = []
            cf_reqs: List = []
            has_crop_flags: List[bool] = []  # per-sample: True iff at least one usable crop
            first_crop_paths: List[Optional[str]] = []  # per-sample: path of first crop, else None

            for ex in inputs:
                msgs = list(ex.get("messages") or [])
                imgs = ex.get("images") or []
                # Extract concrete file paths from the {"bytes":..., "path":...}
                # rollout format used by _generate_completions.
                img_paths = [_path_of(x) for x in imgs] if isinstance(imgs, list) else []
                # Sanitize: keep only entries with an actual file on disk. If
                # the original meme (index 0) is missing, drop the sample from
                # TFA entirely and keep the shape via a 1-image fallback so
                # the batch encoder does not choke.
                good_paths = [p for p in img_paths if p and os.path.isfile(p)]
                if not good_paths:
                    # Should be extremely rare; treat as no-tool and mark None
                    # (TFAToolORM will return 0 for this sample).
                    ex["tfa_D_per_sample"] = None
                    orig_reqs.append(InferRequest(messages=msgs, images=[]))
                    cf_reqs.append(InferRequest(messages=msgs, images=[]))
                    has_crop_flags.append(False)
                    first_crop_paths.append(None)
                    _n_skip += 1
                    continue

                # orig uses the true image sequence
                orig_reqs.append(InferRequest(messages=msgs, images=list(good_paths)))

                # cf: replace the FIRST sandbox crop (index 1) with a same-size
                # white PNG. If there is no crop (len<2), cf == orig -> D_t == 0.
                if len(good_paths) >= 2:
                    try:
                        first_crop = good_paths[1]
                        white_path = get_white_image_of_same_size(first_crop, white_root)
                        cf_paths = list(good_paths)
                        cf_paths[1] = white_path
                        cf_reqs.append(InferRequest(messages=msgs, images=cf_paths))
                        has_crop_flags.append(True)
                        first_crop_paths.append(first_crop)
                    except Exception as _we:
                        print(f"[tfa][warn] rank {_r}: could not build cf for a "
                              f"sample ({type(_we).__name__}: {_we}); using orig "
                              f"as cf (D_t will be ~0).", flush=True)
                        cf_reqs.append(InferRequest(messages=msgs, images=list(good_paths)))
                        has_crop_flags.append(False)
                        first_crop_paths.append(None)
                        _n_fail += 1
                else:
                    # No crop -> cf == orig -> D_t == 0 by construction
                    cf_reqs.append(InferRequest(messages=msgs, images=list(good_paths)))
                    has_crop_flags.append(False)
                    first_crop_paths.append(None)
                    _n_skip += 1

            # -------- Encode + collate BOTH batches (same shapes guaranteed) --------
            with self._template_context(template):
                orig_encs = [template.encode(r) for r in orig_reqs]
                cf_encs = [template.encode(r) for r in cf_reqs]
                orig_batch = to_device(template.data_collator(orig_encs), self.model.device)
                cf_batch = to_device(template.data_collator(cf_encs), self.model.device)

            # Compute logits_to_keep from orig labels (completion length),
            # then align cf batch to the same key set.
            orig_labels = orig_batch.pop("labels")
            _cf_labels = cf_batch.pop("labels", None)
            del _cf_labels
            _lk_tensor = orig_labels.shape[-1] - torch.ne(orig_labels, -100).int().argmax(-1)
            logits_to_keep = int(_lk_tensor.max().item())
            if logits_to_keep <= 0:
                print(f"[tfa][warn] rank {_r}: logits_to_keep={logits_to_keep} "
                      f"(no completion tokens); marking all D_t=None", flush=True)
                # Every sample fails; still do 2 dummy forwards to keep ZeRO-3
                # symmetric. Use logits_to_keep=1 to keep the calls valid.
                logits_to_keep = 1

            # Build the extra keys _get_per_token_logps expects. The path
            # inside ms-swift ignores completion_mask/advantages/etc when they
            # are missing, but we still need logits_to_keep.
            orig_batch["logits_to_keep"] = logits_to_keep
            cf_batch["logits_to_keep"] = logits_to_keep
            orig_batch["completion_mask"] = (orig_labels[:, -logits_to_keep:] != -100)
            cf_batch["completion_mask"] = orig_batch["completion_mask"]

            # -------- Two symmetric forwards --------
            with torch.no_grad():
                orig_logps = self._get_per_token_logps(self.model, orig_batch)  # (B, K)
                cf_logps = self._get_per_token_logps(self.model, cf_batch)      # (B, K)

            # -------- Per-sample D_t over the post-tool window --------
            # completion segment starts at (L - logits_to_keep); left-padded.
            orig_input_ids = orig_batch["input_ids"]                         # (B, L)
            cf_input_ids = cf_batch["input_ids"]                             # (B, L)
            completion_ids = orig_input_ids[:, -logits_to_keep:]             # (B, K)
            completion_mask = orig_batch["completion_mask"]                  # (B, K)

            # Diagnostics: collect raw D_t values and window lengths so we
            # can see whether the eps/weight defaults are calibrated for the
            # current policy's actual distribution. Cheap (a few floats).
            _dbg_D: List[float] = []
            _dbg_wl: List[int] = []
            _dbg_nfall = 0   # samples that fell into the fallback window
            _dbg_nmis = 0    # samples with mismatched orig/cf input_ids

            # If the two batches disagree on token count, we cannot compute
            # D_t at all (they'd be different sequences). Flag it loudly.
            if orig_input_ids.shape != cf_input_ids.shape:
                print(f"[tfa][error] rank {_r}: orig/cf input_ids shape "
                      f"MISMATCH: {tuple(orig_input_ids.shape)} vs "
                      f"{tuple(cf_input_ids.shape)}. Marking all D_t=None.",
                      flush=True)
                for i in range(len(inputs)):
                    inputs[i]["tfa_D_per_sample"] = None
                # jump past the per-sample loop
                del orig_batch, cf_batch, orig_logps, cf_logps
                raise RuntimeError("orig/cf input_ids shape mismatch")

            for i in range(len(inputs)):
                if not has_crop_flags[i]:
                    # No crop or cf-build failed: D_t undefined (bucket 0 later)
                    inputs[i]["tfa_D_per_sample"] = None
                    _n_zero += 1
                    continue
                try:
                    # Only look at the real completion tokens (drop right-padding).
                    _k_valid = int(completion_mask[i].sum().item())
                    if _k_valid <= 2:
                        inputs[i]["tfa_D_per_sample"] = None
                        _n_zero += 1
                        continue
                    _comp_ids = completion_ids[i][:_k_valid]

                    # SANITY: orig and cf input_ids must match token-for-
                    # token for this sample (only vision features differ).
                    # If they don't match, D_t is comparing DIFFERENT
                    # sequences -> garbage in the -2 to -5 nats/token range
                    # (which is exactly what we observed in the log).
                    # Skip and mark None so the ORM returns 0 for this row.
                    if not torch.equal(orig_input_ids[i], cf_input_ids[i]):
                        _dbg_nmis += 1
                        # print a targeted diagnostic for the first mismatch
                        # this step so we know how they diverge.
                        if _dbg_nmis == 1:
                            _diff = (orig_input_ids[i] != cf_input_ids[i]).sum().item()
                            _o_nonpad = int((orig_input_ids[i] != 0).sum().item())
                            _c_nonpad = int((cf_input_ids[i] != 0).sum().item())
                            print(f"[tfa][warn] rank {_r}: sample {i} orig/cf "
                                  f"input_ids DIFFER at {_diff} positions "
                                  f"(orig_nonpad={_o_nonpad}, cf_nonpad={_c_nonpad}). "
                                  f"D_t is meaningless; skipping.",
                                  flush=True)
                        inputs[i]["tfa_D_per_sample"] = None
                        _n_fail += 1
                        continue

                    # Locate window on the FULL (prompt+completion) token
                    # sequence, not just the completion segment. The
                    # ``<sandbox_output>...</sandbox_output>`` block is
                    # injected by the sandbox tool and, under ms-swift's
                    # template, ends up on the PROMPT side of the labels
                    # (its tokens are masked with -100). Slicing to just
                    # the completion made ``locate_post_tool_windows``
                    # fail 75%% of the time in the 20260804_201817 run,
                    # and its fallback ("last K tokens of completion") is
                    # dominated by the ``<answer>...</answer>`` decision
                    # window, which is HUGELY image-sensitive -> D_t=-2..-5
                    # nats/token garbage. Fix: search on the full sequence
                    # and map back to completion coords.
                    _amask = orig_batch.get("attention_mask")
                    if _amask is not None:
                        _L_full = int(_amask[i].sum().item())
                    else:
                        _L_full = int(orig_input_ids.shape[1])
                    # left-padded: real tokens are the LAST _L_full entries
                    _full_ids = orig_input_ids[i][-_L_full:] if _L_full > 0 else orig_input_ids[i]
                    _L_prompt = _L_full - _k_valid   # prompt tokens (labels=-100)

                    _got_window = False
                    try:
                        wins = locate_post_tool_windows(
                            input_ids=_full_ids,
                            tokenizer=tokenizer,
                            n_tools=1,
                        )
                        s_full, e_full = wins[0]
                        # Map full-sequence coords -> completion-only coords
                        s = s_full - _L_prompt
                        e = e_full - _L_prompt
                        # If the window ends before completion starts, the
                        # </sandbox_output> is followed IMMEDIATELY by
                        # </answer> with no reasoning in between -> skip.
                        if e <= 0:
                            inputs[i]["tfa_D_per_sample"] = None
                            _n_zero += 1
                            continue
                        # If the window starts inside the prompt, clamp to
                        # the beginning of the completion (i.e. the whole
                        # completion up to <answer> is the post-tool win).
                        s = max(0, s)
                        _win_from = "sbox"
                        _got_window = True
                    except Exception:
                        # Do NOT fall back to "last K tokens" any more:
                        # that window is dominated by <answer>...</answer>,
                        # which is confounded with the classification
                        # decision and produces massively negative D_t.
                        # Just skip this sample (D_t = None -> ORM 0).
                        inputs[i]["tfa_D_per_sample"] = None
                        _n_fail += 1
                        _dbg_nfall += 1
                        continue
                    ...
                    if not _got_window:
                        ...
                        _n_fail += 1
                        continue
                    # Clamp to real completion length
                    s = max(0, min(s, _k_valid - 1))
                    e = max(s + 1, min(e, _k_valid))
                    # Apply user-configurable K cap
                    if K_default > 0 and (e - s) > K_default:
                        e = s + K_default
                    # -------- CRITICAL: index-space alignment --------
                    # ``orig_logps`` has shape (B, K_max) where K_max ==
                    # logits_to_keep == max completion length in the batch.
                    # ``_get_per_token_logps`` does ``input_ids[:, -K_max:]``
                    # to grab logps. When K_max > _k_valid (i.e. THIS
                    # sample is shorter than the batch's longest), the
                    # first (K_max - _k_valid) positions of ``orig_logps[i]``
                    # are logps for PROMPT-TAIL tokens (image tokens,
                    # system prompt tail, etc.), NOT completion tokens.
                    # Using ``[s:e]`` directly aliases completion-local
                    # coord ``s`` onto prompt-tail position ``s``,
                    # which is catastrophically wrong (prompt-tail image
                    # tokens have HUGE image-vs-white-image logp gaps,
                    # producing the -23 nats/token orig_logp we saw in
                    # the 20260805_110300 deep diagnostic).
                    # Fix: shift by ``offset = K_max - _k_valid``.
                    _K_max = int(orig_logps.shape[1])
                    _offset = _K_max - _k_valid
                    _s_lp = s + _offset
                    _e_lp = e + _offset
                    delta = (orig_logps[i, _s_lp:_e_lp] - cf_logps[i, _s_lp:_e_lp]).to(torch.float64)
                    if delta.numel() == 0:
                        inputs[i]["tfa_D_per_sample"] = None
                        _n_zero += 1
                        continue
                    D_t = float(delta.mean().item())
                    inputs[i]["tfa_D_per_sample"] = [D_t]
                    _dbg_D.append(D_t)
                    _dbg_wl.append(int(e - s))
                    if D_t > 0:
                        _n_pos += 1
                    elif D_t < 0:
                        _n_neg += 1
                    ...
                    # -------- Per-sample DEEP diagnostic --------
                    # Print for (a) the FIRST non-skip sample this rank
                    # sees this step (always, for calibration), and (b)
                    # any extreme |D_t|>0.5 sample (to catch outliers).
                    # This surfaces window boundaries, decoded window
                    # text, absolute logp levels (not just the diff), and
                    # per-token diff quantiles. Without this we cannot
                    # tell "-2 nats/token" apart from "window points at
                    # the wrong span" vs "logps themselves are weird".
                    _should_deep = (len(_dbg_D) == 1) or (abs(D_t) > 0.5)
                    if _should_deep:
                        try:
                            _o_win_ids = orig_input_ids[i][-_L_full:][s_full:e_full]
                            _o_logp_win = orig_logps[i, _s_lp:_e_lp].to(torch.float64)
                            _c_logp_win = cf_logps[i, _s_lp:_e_lp].to(torch.float64)
                            _o_mean = float(_o_logp_win.mean().item())
                            _c_mean = float(_c_logp_win.mean().item())
                            _diff = _o_logp_win - _c_logp_win
                            _diff_sorted = _diff.sort().values
                            _dn = _diff_sorted.numel()
                            _p10 = float(_diff_sorted[int(_dn * 0.1)].item()) if _dn > 0 else float("nan")
                            _p50 = float(_diff_sorted[int(_dn * 0.5)].item()) if _dn > 0 else float("nan")
                            _p90 = float(_diff_sorted[int(_dn * 0.9)].item()) if _dn > 0 else float("nan")
                            _diff_min = float(_diff_sorted[0].item()) if _dn > 0 else float("nan")
                            _diff_max = float(_diff_sorted[-1].item()) if _dn > 0 else float("nan")
                            _text_tok = getattr(tokenizer, "tokenizer", tokenizer)
                            _win_text = _text_tok.decode(
                                _o_win_ids.tolist(), skip_special_tokens=False
                            )
                            _wt_show = _win_text if len(_win_text) <= 120 else (
                                _win_text[:60] + " ... " + _win_text[-60:]
                            )
                            _wt_show = _wt_show.replace("\n", "\\n")
                            print(
                                f"[tfa][deep] rank {_r} sample {i}: "
                                f"D_t={D_t:+.4f} win_len={e-s} "
                                f"| L_full={_L_full} L_prompt={_L_prompt} "
                                f"k_valid={_k_valid} K_max={_K_max} "
                                f"offset={_offset} "
                                f"| s_full={s_full} e_full={e_full} "
                                f"s_comp={s} e_comp={e} "
                                f"s_lp={_s_lp} e_lp={_e_lp} "
                                f"| logp_mean orig={_o_mean:+.4f} cf={_c_mean:+.4f} "
                                f"| diff[min/p10/p50/p90/max]="
                                f"[{_diff_min:+.4f}/{_p10:+.4f}/{_p50:+.4f}/"
                                f"{_p90:+.4f}/{_diff_max:+.4f}] "
                                f"| win_text={_wt_show!r}",
                                flush=True,
                            )
                        except Exception as _dpe:
                            print(f"[tfa][deep-err] rank {_r} sample {i}: "
                                  f"failed to emit deep diagnostic: "
                                  f"{type(_dpe).__name__}: {_dpe}",
                                  flush=True)
                except Exception as _de:
                    print(f"[tfa][warn] rank {_r}: sample {i} D_t compute "
                          f"failed ({type(_de).__name__}: {_de}); D_t=None",
                          flush=True)
                    inputs[i]["tfa_D_per_sample"] = None
                    _n_fail += 1

            # -------- D_t distribution summary --------
            # One line per rank per step: raw D_t stats + window length stats.
            # Compare against TFA_EPSILON (=0.01 by default) to decide if the
            # threshold is well-calibrated.
            if _dbg_D:
                import statistics as _stats
                _d_min = min(_dbg_D)
                _d_max = max(_dbg_D)
                _d_mean = sum(_dbg_D) / len(_dbg_D)
                _d_std = _stats.pstdev(_dbg_D) if len(_dbg_D) > 1 else 0.0
                _n_gt_eps = sum(1 for d in _dbg_D if d > 0.01)
                _n_lt_neg_eps = sum(1 for d in _dbg_D if d < -0.01)
                _wl_min = min(_dbg_wl)
                _wl_max = max(_dbg_wl)
                _wl_mean = sum(_dbg_wl) / len(_dbg_wl)
                print(
                    f"[tfa][dist] rank {_r}: n={len(_dbg_D)}  "
                    f"D_t[min/mean/max/std]=[{_d_min:+.4f}/{_d_mean:+.4f}/"
                    f"{_d_max:+.4f}/{_d_std:.4f}]  "
                    f"|>+0.01|={_n_gt_eps}  |<-0.01|={_n_lt_neg_eps}  "
                    f"win_len[min/mean/max]=[{_wl_min}/{_wl_mean:.1f}/{_wl_max}]  "
                    f"nfall={_dbg_nfall}  nmis={_dbg_nmis}",
                    flush=True,
                )

            # Free the (B, L, V)-sized softmax intermediates ASAP
            del orig_batch, cf_batch, orig_logps, cf_logps
        except Exception as _outer_e:
            # Outer safety net: if the batch-symmetric TFA path itself blows
            # up on THIS rank, we still need every other rank to see the
            # same number of collectives on the current step. There is no
            # clean way to recover mid-flight, so we mark all D_t=None and
            # let the ORM return 0. But this branch means the SUBSEQUENT
            # ZeRO-3 collectives (gather etc.) are likely already desynced
            # -> in practice training will hang on the next step. Print a
            # loud error so operators notice.
            print(f"[tfa][error] rank {_r}: batch-symmetric TFA path "
                  f"FAILED before both forwards completed: "
                  f"{type(_outer_e).__name__}: {_outer_e}", flush=True)
            import traceback as _tb
            _tb.print_exc()
            for ex in inputs:
                ex["tfa_D_per_sample"] = None
        finally:
            # We never mutated template.mode (see above), so there is no
            # mode to restore. Only restore the transient ``.model`` back-
            # pointer that qwen._get_position_ids may have relied on.
            template.model = template_model_backup

        print(
            f"[tfa][compute] rank {_r}: step done in {time.time()-_t_start:.1f}s | "
            f"pos={_n_pos} neg={_n_neg} zero={_n_zero} skip={_n_skip} "
            f"fail={_n_fail} | n_samples={len(inputs)}",
            flush=True,
        )
    ...
    def _get_per_token_logps(self, model, inputs):
        ...
        # NOTE(safety-thyme): the naive path was
        #     logits = logits / self.temperature
        #     return selective_log_softmax(logits, input_ids)
        # which materializes ``[B, K+1, V]`` bf16 (~5 GB with K~3200, V=152064)
        # -> OOM on 95GB H20 when vllm colocate takes ~40GB and a slightly
        # longer completion arrives (observed around step 733 twice in a row).
        #
        # Fix: chunk along the sequence dimension so we never keep more than
        # ``_CHUNK * V`` elements alive at once. autograd still works because
        # each chunk is a normal differentiable subgraph; PyTorch stitches the
        # gradients from all chunks. Peak activation drops by ``K / _CHUNK``x
        # (K~3200, _CHUNK=256 -> 12x, ~400MB peak).
        import torch.nn.functional as _F
        _T = float(self.temperature)
        _CHUNK = int(os.environ.get('SAFETY_THYME_LOGP_CHUNK', '256'))
        _per_token_logps_rows = []
        for _row_logits, _row_ids in zip(logits, input_ids):  # over batch dim
            _K = _row_logits.shape[0]
            _row_out_parts = []
            for _s in range(0, _K, _CHUNK):
                _e = min(_s + _CHUNK, _K)
                # slice is a view over row_logits; the divide below allocates
                # only ``[_CHUNK, V]`` and is immediately consumed by
                # log_softmax + gather, then freed at the end of the loop.
                _chunk = _row_logits[_s:_e] / _T
                _chunk_lp = _F.log_softmax(_chunk, dim=-1)
                _chunk_ptl = _chunk_lp.gather(
                    dim=-1, index=_row_ids[_s:_e].unsqueeze(-1)
                ).squeeze(-1)
                _row_out_parts.append(_chunk_ptl)
                del _chunk, _chunk_lp
            _per_token_logps_rows.append(torch.cat(_row_out_parts, dim=0))
            del _row_out_parts
        return torch.stack(_per_token_logps_rows, dim=0)
